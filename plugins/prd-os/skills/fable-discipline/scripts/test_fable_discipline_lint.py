...
CASES = [
    ...
    ("test_golden_fixture.py",
     ...
     0),
    # ASK-542: isolation is decided per path SEGMENT. As substrings, `temp`
    # hid inside `attempts` and `contempt`, so both unsafe paths PASSED.
    ("test_attempts_dir.py",
     'import sqlite3\ndef test_x():\n    sqlite3.connect("/repo/attempts/prod.db")\n',
     2),
    ("test_contempt_dir.py",
     'import sqlite3\ndef test_x():\n    sqlite3.connect("/repo/contempt/prod.db")\n',
     2),
    # ...and the segment match still passes the real safe shapes.
    ("test_tmp_segment.py",
     'import sqlite3\ndef test_x():\n    sqlite3.connect("/tmp/run1/prod.db")\n',
     0),
    ("test_subword_fixture.py",
     'import sqlite3\ndef test_x():\n    sqlite3.connect("tests/test_fixtures/prod.db")\n',
     0),
    ("test_macos_tempdir.py",
     'import sqlite3\ndef test_x():\n    sqlite3.connect("/var/folders/ab/T/prod.db")\n',
     0),
    ("app.py",
     'import sqlite3\nsqlite3.connect("investigations/data/investigations.db")\n',
     0),
