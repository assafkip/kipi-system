...
- `/q-draft` - Ad-hoc output generation. The `improve` SKILL is the inverse: critique an outside idea against what this system already has; runs `plugins/kipi-core/skills/improve/scripts/improve_ground.py` first (`already-built` names the file or lesson, `adopt`, or `skip` when the roadmap classifier says what-to-build), corpora via `KIPI_LESSONS_CORPORA` each reported read / missing / unreadable; on demand only
