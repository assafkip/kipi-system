...
- [ ] **Loop opened for every outbound action from this debrief:**
  - [ ] If next step involves sending something (deck, email, DM): use the `loop_open` MCP tool with type=<materials_sent|email_sent|dm_sent>, target="Person", context="What was sent", follow_up_text="Follow-up text..."
  - [ ] If next step involves waiting for their response: use the `loop_open` MCP tool with type="debrief_next_step", target="Person", context="What we're waiting for"
  - [ ] If design partner conversion message was sent: use the `loop_open` MCP tool with type="dp_offer_sent", target="Person", context="DP offer context", follow_up_text="Follow-up if no reply..."
