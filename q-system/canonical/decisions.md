...
- **Completion (2026-07-07 cleanup, [SYSTEM-INFERRED]):** dissolution left two loose ends, now fixed. (1) `ktlyst-hub/product-baseline` residue (dangling worktree; gitdir pointed at a deleted `/Users/assafkip/…/product`) — the whole `ktlyst-hub/` shell archive-moved to `_archive/ktlyst-hub-dissolved-2026-07-07`. (2) promoted repo `ktlyst-saas/projects/product` had a dead `file://` origin — repointed to real upstream `github.com/assafkip/ktlyst.git` (fetch verified; single-branch refspec + phantom `feat/v5-canonical-port` tracking cleared). Fleet scan: zero dead remotes/worktrees remain.
- **OPEN (awaits [USER-DIRECTED]):** v5 (the LIVE product) has NO remote backup — `feat/v5-canonical-port` was never pushed to GitHub and v5 HEAD is not an ancestor of ktlyst.git `main` (independent line). Decide: push v5 as a branch to ktlyst.git, or stand up a dedicated `ktlyst-saas-product` repo.
- **Date:** 2026-07-07
