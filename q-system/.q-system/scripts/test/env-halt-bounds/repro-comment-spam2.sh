#!/usr/bin/env bash
# REPRO v2: the halt dedupes its PAGE (env_alert_claim across processes) but not
# its per-issue Linear COMMENT. Every halted run posts another
# "**Not attempted.**" comment on the same issue. 3 ticks = 3 comments.
set -uo pipefail
...
echo "--- pages fired across 3 halted runs (deduped): $(grep -c '^NOTIFY ' "$NOTIFY_LOG" 2>/dev/null || echo 0)"
echo "--- Linear commentCreate calls carrying 'Not attempted' (NOT deduped):"
grep -c 'Not attempted' "$WORK/comments.log" 2>/dev/null || echo 0
echo "--- sample:"
head -c 300 "$WORK/comments.log"
