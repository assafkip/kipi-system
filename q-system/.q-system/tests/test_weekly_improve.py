...
def test_one_line_yields_a_proposal_citing_id_and_masked_excerpt(weekly, tmp_path):
    ...
    long_text = ("the brief lists Sana's tickets as mine, change the owner rule so that "
                 "owner:sana never renders as mine again, contact via ops@example.com")
    ...
    assert "ops@example.com" not in msg and "[email]" in msg or "..." in msg
