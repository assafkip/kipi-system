...
**If `{config_dir}/founder-profile.md` contains `{{SETUP_NEEDED}}`, this is a fresh install. Run the setup wizard before doing anything else.**
...
Save answers to `{config_dir}/founder-profile.md`.
...
Save answers to `{data_dir}/my-project/current-state.md` and `{config_dir}/canonical/discovery.md`.
...
- Save integration config to `{config_dir}/enabled-integrations.md`
- Save all database IDs to `{data_dir}/my-project/notion-ids.md`
...
Populate `{data_dir}/my-project/relationships.md`.
...
- `{config_dir}/canonical/discovery.md` = what's been asked and answered
- `{config_dir}/canonical/objections.md` = known pushback + responses
- `{config_dir}/canonical/talk-tracks.md` = proven language
- `{data_dir}/my-project/relationships.md` = people + conversation history
- `{data_dir}/my-project/competitive-landscape.md` = substitute buckets
- `{config_dir}/canonical/lead-lifecycle-rules.md` = when to kill/park/re-engage leads (ENFORCED in morning routine + hitlist generation)
- `{config_dir}/canonical/engagement-playbook.md` = social engagement rules + comment strategy
- `{config_dir}/canonical/market-intelligence.md` = buyer language, category signals, competitive intel, and narrative validation from external content (LinkedIn, Reddit, Medium, X). Auto-populated during lead sourcing and engagement. Read before generating any marketing content or outreach.
...
Read `{config_dir}/voice/voice-dna.md` and `{config_dir}/voice/writing-samples.md` before generating any written content. Apply all rules from `.claude/skills/founder-voice/SKILL.md` including the anti-AI detection patterns.
...
3. **For practitioner/buyer conversations: run the Design Partner Conversion section.** This is MANDATORY. The debrief is not complete until there is a copy-paste message the founder can send to convert the conversation into a design partner trial. Read `{data_dir}/my-project/current-state.md` to map their pain to current capabilities. Output the message and create a follow-up Action in Notion.
...
Adapt to the founder's profile from `{config_dir}/founder-profile.md`. Key accommodations:
