# Build-Craft Auto-Invocation (ENFORCED)

Invoke the `build-craft` skill (kipi-core) BEFORE writing or editing code on any
...
| Trigger | Action |
|---------|--------|
| Building a feature, fixing a bug, writing a script | Read build-craft SKILL.md, then follow its checklist |
| Writing or editing tests | Same. The build-craft-lint hook also blocks tests that touch a live data path |
...
- Pure content/docs (those go through founder-voice, not build-craft)
- Reading or searching only (no edit)

**Relationship to other rules:**
- `coding-standards.md` is the static style baseline. build-craft is the
  procedure layer on top of it.
- `rca-mode.md` is the diagnostic mirror: build-craft is forward (how to build so
...
- The deterministic slice (test isolation) is enforced by the build-craft-lint
