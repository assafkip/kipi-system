...
PY
        ...
        if ! ( cd "$path" && while IFS= read -r -d '' uf; do
            if ! { [ -e "$uf" ] || [ -L "$uf" ]; } && { [ -e "$SNAP/f/$uf" ] || [ -L "$SNAP/f/$uf" ]; }; then
              # The index still lists it -- rsync --delete touched the worktree
              # only -- so this reads the pre-sync tracking state, which is the
              # one the operator cares about.
              if git ls-files --error-unmatch -- "$uf" >/dev/null 2>&1; then
                kind="tracked instance-only; still tracked, nothing changed in git"
              else
                kind="untracked"
              fi
              mkdir -p "$(dirname "$uf")" && cp -a "$SNAP/f/$uf" "$uf" && echo "  restored ($kind): $uf"
            fi
          done < "$SNAP/list" ); then
