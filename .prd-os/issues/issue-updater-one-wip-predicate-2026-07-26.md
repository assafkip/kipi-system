...
required_reviews: []
bypass_check: "test \"$(grep -v '^[[:space:]]*#' kipi-update.sh | grep -c 'is_instance_wip')\" -eq 3 && test \"$(grep -c -F 'cmp -s \"$instance_file\"' kipi-update.sh)\" -eq 1 && grep -q 'refusing to commit unrelated work' kipi-update.sh"
---
